...
Notes:
- **Icons:** auto-pick a default icon by domain; allow manual override (MDI icon set). See the
  icon-rendering decision below.
- **Icon rendering (decided — for later development):** icons are rendered on-device as **raw
  pre-decoded bitmaps**, *not* as a compiled-in LVGL font. Rationale: we ride on ESPHome's `lvgl:`
  and must not edit the generated `lv_conf.h` (it changes between ESPHome releases), so we cannot
  rely on a runtime font/SVG engine (`tiny_ttf`/FreeType are stubbed out: `LV_USE_TINY_TTF=0`) or
  on image decoders (no PNG/JPEG decoder compiled in). What *is* available is `LV_USE_IMAGE` —
  LVGL blits a raw `lv_image_dsc_t` from memory with no decoder.
  - **Format:** **A8** (8-bit alpha mask, 1 byte/px — 32×32 ≈ 1 KB). Color-agnostic: the device
    tints it at runtime with LVGL image **recolor** (`image_recolor` + `image_recolor_opa`), so one
    blob serves both fixed-color and state-color modes and is cached by `name+size` only.
  - **Hosting (decided):** **statically pre-rendered on GitHub Pages**, generated by the `web/`
    CI build (rasterize MDI SVG → A8 `.bin` per icon/size), served alongside the SPA bundle. No
    dynamic/serverless endpoint and no new infra; the icon set is fixed at build time (curate a
    subset or render the full set). Device fetches e.g. `GET <spa_base>/icons/<name>_<size>.bin`
    via the `http_request` component and caches the bytes in PSRAM. Fetch async; assign the image
    on the main loop when bytes arrive (LVGL is main-loop-only).
  - **Interim:** until this lands, widgets map a handful of MDI names to LVGL's built-in symbol
    font (see `badge.cpp` `mdi_to_symbol`); unmapped icons render nothing.
